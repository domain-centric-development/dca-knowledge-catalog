...
- It is **unversioned** by design (it can evolve freely). External consumers need a stable, versioned contract — that is exactly what `IntegrationEvent` (`int version()`) provides.
...
- Forbidden by intent of: [Integration Events must be in events or adapter outgoing event packages](/rule/strategic/integration-events-must-be-in-events-or-adapter-outgoing-event-packages.md) · [Integration Events must have a version field](/rule/advanced/integration-events-must-have-a-version-field.md)
