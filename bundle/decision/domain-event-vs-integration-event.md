...
2. **Must the payload be a stable, versioned wire contract?** Cross-context consumers depend on shape stability → Integration Event (carries `int version()`). Internal listeners can evolve with the model → Domain Event (no version, by rule).
...
| Versioning | forbidden (`version` field not allowed) | required (`int version()`) |
...
`aggregate raises DomainEvent` → in-context listeners react directly → an ACL translator maps it to a versioned `IntegrationEvent` → published to the outside. The domain type never crosses the wire; the versioned contract does.
...
- Rules: [Domain Events that are not Integration Events must not have a version field](/rule/advanced/domain-events-that-are-not-integration-events-must-not-have-a-version-field.md) · [Integration Events must have a version field](/rule/advanced/integration-events-must-have-a-version-field.md) · [Integration Events must be in events or adapter outgoing event packages](/rule/strategic/integration-events-must-be-in-events-or-adapter-outgoing-event-packages.md)
