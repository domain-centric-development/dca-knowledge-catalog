---
type: Guide
title: Repository vs. Store
source: guide
resource: dca-guide/architecture/repository-vs-store.md
tags: [guide, guide]
---

DCA distinguishes two kinds of persistence-shaped output ports. Both `extend OutputPort`, but their **business semantics differ**:

## Sections

- [Repository](/guide/repository-vs-store/repository.md)
- [Store](/guide/repository-vs-store/store.md)
- [Decision matrix](/guide/repository-vs-store/decision-matrix.md)
- [Rules of thumb](/guide/repository-vs-store/rules-of-thumb.md)
- [Why the distinction matters](/guide/repository-vs-store/why-the-distinction-matters.md)
- [Separate domain and persistence model](/guide/repository-vs-store/separate-domain-and-persistence-model.md)
