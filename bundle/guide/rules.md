---
type: Guide
title: Rules
source: guide
resource: dca-guide/architecture/rules.md
tags: [guide, guide]
---

Rules.

## Sections

- [THE FUNDAMENTAL DEPENDENCY RULE](/guide/rules/the-fundamental-dependency-rule.md)
- [DOMAIN LAYER RULES](/guide/rules/domain-layer-rules.md)
- [APPLICATION LAYER RULES](/guide/rules/application-layer-rules.md)
- [ADAPTER LAYER RULES](/guide/rules/adapter-layer-rules.md)
- [INFRASTRUCTURE LAYER RULES](/guide/rules/infrastructure-layer-rules.md)
- [STRATEGIC DESIGN RULES](/guide/rules/strategic-design-rules.md)
- [BOUNDARY CROSSING RULES](/guide/rules/boundary-crossing-rules.md)
- [TESTING RULES](/guide/rules/testing-rules.md)
- [ERROR HANDLING RULES](/guide/rules/error-handling-rules.md)
- [TRANSACTION RULES](/guide/rules/transaction-rules.md)
- [PACKAGING RULES](/guide/rules/packaging-rules.md)
- [Wiring and domain metadata](/guide/rules/wiring-and-domain-metadata.md)
- [Operation boundaries and declared contracts](/guide/rules/operation-boundaries-and-declared-contracts.md)
